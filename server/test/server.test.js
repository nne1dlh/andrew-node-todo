...
describe('POST /todos', () => {
    ...
    describe('GET /todos', () => {
        it('should get all todos', (done) => {
            request(app)
                ...
                .expect(200)
                .expect((resx) => {
                    expect(resx.body.todos.length).toBe(1);
                })
                .end(done);
        });
    });
    ...
    describe('GET /todos/:id', () => {
        ...
        it('should not return task doc created by another user', (done) => {
            request(app)
                .get(`/todos/${todoos[1]._id.toHexString()}`)
                .set('x-auth', users[0].tokens[0].token)
                .expect(404)
                .end(done);
        });
    ...
    describe('DELETE /todos/:id', () => {
        ...
        it('should not delete a task not owned bu user', (done) => {
    ...
    describe('PATCH /todos/:id', () => {
        ...
        it('should not update todo created by another', (done) => {
            var hexId = todoos[0]._id.toHexString();
            var text = "Finish second bench";
            request(app)
            .patch(`/todos/${hexId}`)
        ...
        it('should clear compAt when todo is not complete', (done) => {
            var hexId = todoos[1]._id.toHexString();
            ...
            request(app)
                ...
                .end((err,res) => {
                    ...
                    User.findById(users[1]._id).then((usr) => {
                        expect(usr.tokens[1]).toMatchObject({
                            access: 'auth',
    ...
    describe('DELETE user/me/token', () => {
        it('should remove auth token when loggide out', (done) => {
            request(app)
